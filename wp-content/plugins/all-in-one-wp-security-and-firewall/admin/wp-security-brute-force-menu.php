...
class AIOWPSecurity_Brute_Force_Menu extends AIOWPSecurity_Admin_Menu {
	...
	private function render_rename_login() {
		...
		if (isset($_POST['aiowps_save_rename_login_page_settings'])) { // Do form submission tasks
			...
			if (empty($_POST['aiowps_login_page_slug']) && isset($_POST["aiowps_enable_rename_login_page"])) {
				...
				if ('admin' == $aiowps_login_page_slug) {
		...
		$aio_wp_security->include_template('admin/brute-force/rename-login.php', false, array('aiowps_feature_mgr' => $aiowps_feature_mgr, 'home_url' => $home_url));
	}
	...
	private function render_cookie_based_brute_force_prevention() {
		...
		$aio_wp_security->include_template('admin/brute-force/cookie-based-brute-force-prevention.php', false, array('aiowps_feature_mgr' => $aiowps_feature_mgr));
	}
	...
	private function render_captcha_settings() {
		...
		$aio_wp_security->include_template('admin/brute-force/captcha-settings.php', false, array('aiowps_feature_mgr' => $aiowps_feature_mgr, 'supported_captchas' => $supported_captchas, 'default_captcha' => $default_captcha));
	}
	...
	private function render_login_whitelist() {
		...
		$aio_wp_security->include_template('admin/brute-force/login-whitelist.php', false, array('aiowps_feature_mgr' => $aiowps_feature_mgr, 'your_ip_address' => $your_ip_address, 'result' => $result));
	}
	...
	private function render_honeypot() {
		...
		$aio_wp_security->include_template('admin/brute-force/honeypot.php', false, array('aiowps_feature_mgr' => $aiowps_feature_mgr));
	}
