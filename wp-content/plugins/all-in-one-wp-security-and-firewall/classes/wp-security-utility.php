...
class AIOWPSecurity_Utility {
	...
	public static function is_tfa_or_self_plugin_activating() {
		...
		return is_admin() &&
			preg_match('#/admin/plugins.php$#i', $_SERVER['PHP_SELF']) && isset($_GET['plugin']) && (preg_match("/\/two-factor-login.php/", $_GET['plugin']) || preg_match("/all-in-one-wp-security-and-firewall/", $_GET['plugin']));
	}
