	...
	public function is_satisfied() {
		...
		if (!isset($_GET[$brute_force_secret_word])) {
			...
			if (isset($_SERVER['REQUEST_URI']) && '' != $_SERVER['REQUEST_URI'] && !hash_equals($brute_force_secret_cookie_val, hash_hmac('md5', $brute_force_secret_word, $brute_force_cookie_salt))) {
				...
				$is_admin_or_login = (false != strpos($_SERVER['REQUEST_URI'], 'admin') || false != strpos($_SERVER['REQUEST_URI'], 'wp-login') || ('' != $login_page_slug && false != strpos($_SERVER['REQUEST_URI'], $login_page_slug))) ? 1 : 0;
				
				// admin side ajax called
				$is_admin_ajax_request = ('1' == $prevent_ajax_exception && false != strpos($_SERVER['REQUEST_URI'], 'admin/admin-ajax.php')) ? intval($prevent_ajax_exception) : 0;
